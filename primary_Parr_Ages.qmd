...
-   include tables (entirely) and figure captions (not image) directly
    after the paragraph where they are first cited in the text.
-   please use "Fig" instead of "Figure". This has been made the default
    in this Quarto format.
...
\[need to define jargon: parr vs presmolt, brood year, smolt year,
adults vs spawners,\]\
...
and in Osoyoos Lake (refs, Fig.1 ). Both result from successful
...
Smolts emigrating from Osoyoos Lake were captured in a Rotary Screw Trap (RST) 
positioned between the North and South basin of Osoyoos Lake (i.e., Osoyoos 
Narrows). An RST is a stationary fish sampling tool that is installed around the 
river thalweg to randomly and passively capture small, riverine fishes. The 
Osoyoos Narrows RST has been used to annually monitor smolting salmonids as they
depart from Osoyoos Lake since [<span style = "color:red;">year</span>], where
members of the Okanagan Nation Alliance (ONA) have been collecting annual smolt 
biometric data as well as monitoring smolt timing and, more recently, deploying 
PIT-tagged smolts to track downstream migration and survival rates. 

As the smolt monitoring data at the Osoyoos Narrows RST typically lack otolith 
or scale based age data, we used size, length and size-at-length data to 
estimate the annual relative fractions of age 1.0 and age 2.0 smolts leaving 
Osoyoos Lake. 

*[methods for fitting double gaussian model here - dealing w/ truncated lengths, incorporating condition factors?]*
