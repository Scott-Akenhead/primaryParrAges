---
title: "Reconstructing age composition of Sockeye Salmon (Oncorynchus nerka) parr in Osoyoos Lake (British Columbia, Canada)."
format:
  plos-pdf:
    keep-tex: true
...
author:
  - name: Scott A. Akenhead
    email: scott@s4s.com
    affiliations: 
        - id: https://orcid.org/0000-0003-1218-3118
  ...
  - name: Braden Judson
    ...
    affiliations:
        - id: https://orcid.org/0000-0002-4432-9372
          name: Pacific Biological Station
          department: Fisheries and Oceans Canada
          address: 3190 Hammond Bay Road
          city:  Nanaimo
...
Osoyoos Lake Sockeye Salmon (OLSS) represent one of only two
self-sustaining Sockeye conservation units in the Columbia River
Watershed. Over a century of habitat degradation and the erection of
hydroelectric and agricultural dams has extirpated wild Sockeye from
much of their historic range. However, after population declines in the
1990's, efforts from the Okanagan Nation Alliance (ONA) have spurred
various habitat restoration projects, a supplementation hatchery, and
two translocation projects in the Okanagan basin. Consequently, Okanagan
Sockeye Salmon abundances have increased dramatically over the last two
decades and therefore considerable efforts are being invested into
population monitoring and research assessing the lake's capacity for
spawning and rearing [@akenheadscotta.; @wilkinson2016].

Fig.1 map locating WE and OSO in Columbia Watershed in North America.

Sockeye parr abundances are a prime focus of ongoing population
monitoring. Monitoring activities are restricted to the North Basin of
Osoyoos Lake (Fig. #) as historic surveys in the South Basin observed
low abundances and South Basin water temperatures are generally
considered inhospitable for rearing \[refs\]. However, the contemporary
assumption of low parr abundance in the South Basin is an eplicit source
of uncertainty in this study, although water temperature data suggest
that this source of error is likely small. Parr, however, must pass
through the South Basin on their outward spring migration.

The life histories of OLSS are well described (refs) and involve a
number of discrete habitats:
...
5.  September--March: parr change into presmolts; not all parr change
    ...
    thought to be small.
6.  April--May: presmolts emigrate from the lake as smolts and traverse
    eight hydroelectric dams and over 600km of the Columbia River before
...
![Life history of Sockeye Salmon in Lake Okanagan. Numbers (0 to 1430)
are the approximate age in days at each life stage: egg, fry,. Important
habitats factors potentially affecting survival rates in each life stage
are identified (blue circles), for instance survival during transit of
10 dams as smolts migrate downstream to the Pacific Ocean and returning
adults migrate upstream to the spawning habitat. This is not a fixed
schedule: varying fractions of marine juveniles return after 2, 3, or 4
summers in the ocean; similarly, parr emigrate after 1 or 2 summers in
the
lake.](figures/life%20cycle%20OSO%20SOX%20stages%20days%20factors%202023-04-23.png){#fig:lifehistory}
...
## Data sources and age composition estimates

Uninterrupted time series of abundance estimates were available for
spawner abundance from counting live and dead spawners, 199x--2022
(ref), parr and presmolts from acoustic surveys of the North Basin of
Okanagan Lake 1998-2021 (ref), and adult returns at Bonneville Dam.
...
Smolts emigrating from Osoyoos Lake were captured in a Rotary Screw Trap
(RST) positioned between the North and South basin of Osoyoos Lake
(i.e., Osoyoos Narrows). An RST is a stationary fish sampling tool that
is installed around the river thalweg to randomly and passively capture
small, riverine fishes. The Osoyoos Narrows RST has been used to
annually monitor smolting salmonids as they depart from Osoyoos Lake
since \[[year]{style="color:red;"}\], where members of the Okanagan
Nation Alliance (ONA) have been collecting annual smolt biometric data
as well as monitoring smolt timing and, more recently, deploying
PIT-tagged smolts to track downstream migration and survival rates.

As the smolt monitoring data at the Osoyoos Narrows RST typically lack
otolith or scale based age data, we used size, length and size-at-length
data to estimate the annual relative fractions of age 1.0 and age 2.0
smolts leaving Osoyoos Lake.

*\[methods for fitting double gaussian model here - dealing w/ truncated
lengths, incorporating condition factors?\]*
...
ignoring sex ratio and fecundity.
...
### Returning Adults Abundance

after marine fishing
...
Return Year &Age 1.1 &Age 1.2 &Age 1.3 &Age 1.4 &Age 2.1 &Age 2.2 &Age 3.1 \\\midrule
...
from Analysis OSO Parr and Spawners.qmd define parr vs presmolts

### Parr weight and length
...
days apart throughout the peak of the spawning season (Spawning Grounds
Data Report, unpublished data). Briefly, the abundance of spawners
typically increases from a low number, plateaus at some annual maximum
and proceeds to decline as spawners die. Plotting these data yields a
trapezoidal shape, which is used in conjunction with an average fish
residency time (11 days for Okanagan Sockeye Salmon; Hyatt et al., 2010,
Perrin and Irvine, 1990) to estimate the cumulative abundance of
...
preceding smolt emigration. Equation 1 is applied as cumulative survival
(the integral of the changing mortality rates) from day zero to a
preceding or following date. We chose fall equinox as day zero ($t = 0$)
in part because that was close to the median date for all surveys.
Predictions for observations of parr in summer (before day zero) are
greater than at at day zero, and predictions for presmolts in winter are
lower abundance values. The ATS observations were weighted as an inverse
function of their precision; we used the inverse of available 95% CLs,
but truncated for values above the median to avoid overemphasizing
spuriously precise data. 

### **Parr and Presmolt Abundance ~~Density and diversity of limnetic nerkids~~**
...
**\[was each transect sampled twice each night?\]**. ~~In 2006, survey
crews transitioned from a Simrad EY-M echosounder (70 kHz, 75W) to a
Biosonics DT-X echosounder (200 kHz, 300W) although these values were
tightly correlated and enabled calibration between the two echosounders
(R2 = 0.94, n = 14;~~ see Hyatt et al. (2017) for details of survey
procedures and abundance estimates; McLennan and Simmonds (1992) for
acoustics methods in fisheries science.
...
RST. Goal is PIT-tagging and biometrics, not pop size estimation. Need
to update narrowed in map to show OSO narrows RST site.

### **Smolt ages from spawner ages**
...
### Principle components to merge spawner abundance indices

asdf

### Mortality model to estimate annual abundance of parr
...
abundance $N$ by days $t$ within a *Smolt_Year*.
...
Applied to many years (indexed as *j* ), with fixed *m,* this regression
estimates the initial abundance $N_0$ at some arbitrary time from all of
the observations in that Smolt_Year. Opening *m* to vary by day or year
according to various factors (indexed as *i* ) such as water flows,
mitigation of extreme flows by FWMT, smolt size, smolt density, and
perhaps trends across years:
...
be proposed, *e.g.* $m=f(length)$, but the simplest is an exponential
decrease with time:
...
where *m~0~* is the rate (per day) at some day *t=0*, and *m~1~*
determines how quickly *m~t~* declines. This allows *m* to be invariant,
as preceding, if $m_1=0$, or to decline in proportion to days of life if
$m_1>0$.

Note that *t* can be negative, allowing an ecologically meaningful, or
practical, choice for day *t=0.* Example: if
$t=(-100,0,100) \text{ and } m_0=0.005, ~ m_1=0.005; \text{ then } m_t= (0.00305,0.0025,0.00205)$.
The survivals for those days would be 0.9918, 0.9950, 0.9970, allowing
integration as the cumulative product of daily survival.

Fitting this non-linear model requires searching for parameters that
minimize a criterion for fit; a steepest-descent to minimize SSQ (sum of
squared residuals). The model has two parts:\
(1) mortality rates according to day of the year, then\
(2) for each smolt year, cumulative survivals from a fitted $N_0$ at Day
Zero (Fall Equinox) to each survey day is the prediction of parr
abundances
...
### Smolt age composition from length frequencies

asdf

### Parr age composition estimated from adult ages

asdf

### Parr ages from a model of four life stages
...
Parr and Presmolt Abundance

!\[Fitted parr abundance at day 204 from regression model utilizing all
surveys.\](fitted%20parr%20from%20survival%20model.png){#parr-fitted
width="6in"}

residuals versus observed and residuals versus weight.

!\[Predicted versus observed from parr mortality model. Note that effect
of residuals depends on regression
weights.\](parr%20reg%20predicted%20vs%20observed.png){#obs-exp
width="4in"}

!\[Residuals versus regression weight from parr mortality model. The
effect of the largest residuals is reduced by low
weight.\](parr%20reg%20resid%20vs%20weight.png){#res-wt width="6in"}

## Parr and presmolts abundance

asdf

```{r}
#| label: tbl-regr
#| tbl-cap: Parr abundance estimates by year at a standardized day (fall equinox) from a model of time-varying mortality rate. The motality rate and pattern is the same for all years. 
#| echo: false
regest = readRDS("parr regression est and stderr.RData")
knitr::kable(regest[3:26,], digits=c(0,3,3), align='c', row.names=FALSE, 
  col.names = c('Smolt Year','Abundance','Std. Err.'))

```

asdf

## Spawners to parr regression
...
%\hline
\toprule
\textbf{} &\multicolumn{3}{c}{\textbf{Age 1.0}} &\multicolumn{3}{c}{\textbf{Age 2.0}} \\\cmidrule{2-7}
\textbf{Year} &\textbf{Proportion} &\textbf{Mean} &\textbf{Stdev} &\textbf{Proportion} &\textbf{Mean} &\textbf{Stdev} \\\midrule
...
Note frequent bimodal distributions, indicating abundant age 2.0 smolts. Abundant age 2.0 smolts in some years can be recognized as siblings of preceding abundant age 1.0 smolts(2018-2019, 2010-2011).}
\label{fig04-22-1356}
\end{figure}
```
...
Fryer, J., Kelsey, D., Wright, H., Folks, S., Bussanich, R., Hyatt, K.,
Selbie, D., Stockwell, M., 2018. Studies into factors limiting the
abundance of Okanagan and Wenatchee Sockeye Salmon in 2015. Columbia
River Intertribal Fish Commission (CRITFC. Technical Report.

Fryer, J., Wright, H., Folks, S., Hyatt, K., 2011. Studies into factors
limiting the abundance of Okanagan and Wenatchee sockeye salmon.
Columbia River Intertribal Fish Commission (CRITFC. Technical Report.

Hyatt, K., Alexander, C.A.D, 2005. The Okanagan Fish-Water Management
(OKFWM) Tool: Results of a 25 year retrospective analysis. Prepared for
Canadian Okanagan Basin Technical Working Group, Kamloops, BC. 24 pp.
